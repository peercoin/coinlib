...
int secp256k1_musig_nonce_process(const secp256k1_context* ctx, secp256k1_musig_session *session, const secp256k1_musig_aggnonce  *aggnonce, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache, const secp256k1_pubkey *adaptor) {
    ...
    if (adaptor != NULL) {
        secp256k1_ge adaptor_pt;
        secp256k1_gej first_ptj;

        /* Load adaptor point */
        if (!secp256k1_pubkey_load(ctx, &adaptor_pt, adaptor)) {
            return 0;
        }

        /* Obtain Jacobian point of first nonce */
        secp256k1_gej_set_ge(&first_ptj, &aggnonce_pts[0]);

        /* Add adaptor point to first nonce which will be included in overall nonce */
        secp256k1_gej_add_ge_var(&first_ptj, &first_ptj, &adaptor_pt, NULL);

        /* Set the first nonce affine point */
        secp256k1_ge_set_gej(&aggnonce_pts[0], &first_ptj);
